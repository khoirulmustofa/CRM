...
watch(appState, (newVal) => {
    try {
        localStorage.setItem("app-theme", JSON.stringify(newVal));
    } catch (e) {
...
export function useLayout() {
    ...
    function toggleDarkMode() {
        ...
        // Auto-select default surface based on mode if none is selected
        if (!appState.value.surface) {
            const defaultSurface = appState.value.darkMode ? "zinc" : "slate";
            updateColors("surface", defaultSurface);
        }
    }
    ...
    function initTheme() {
        ...
        const surfaceName = appState.value.surface || (appState.value.darkMode ? 'zinc' : 'slate');
        const surfaceColor = surfaces.value.find(s => s.name === surfaceName);
        if (surfaceColor) {
            updateSurfacePalette(surfaceColor.palette);
        }
